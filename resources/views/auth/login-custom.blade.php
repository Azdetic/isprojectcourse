...
<body class="min-h-screen bg-white relative overflow-x-hidden">
    ...
    <div class="absolute top-8 right-8 z-20 hidden md:block">
        <a href="{{ route('signup') }}" class="px-6 py-2.5 border border-white/30 bg-white/10 backdrop-blur-sm text-white rounded-full text-sm font-bold hover:bg-white hover:text-red-900 transition-all shadow-lg">
            SIGN UP
        </a>
    </div>
            ...
            <form action="{{ route('login.store') }}" method="POST" class="space-y-5">
                ...
                <div class="flex items-center justify-between text-sm mt-2">
                    <label class="flex items-center space-x-2 cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-4 h-4 border-gray-300 rounded text-red-600 focus:ring-red-500 transition">
                        <span class="text-gray-500 text-xs font-semibold group-hover:text-gray-700">Remember Me</span>
                    </label>
                    <a href="#" class="text-red-700 font-bold text-xs hover:underline">Forgot Password?</a>
                </div>

                <button type="submit" class="w-full bg-[#B91C1C] hover:bg-red-800 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-red-900/20 transition-all transform active:scale-[0.98] mt-4">
                    LOG IN
                </button>
                ...
                <button type="button" class="w-full bg-white border border-gray-200 text-gray-700 font-bold py-3.5 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all flex items-center justify-center gap-3">
                    ...
                    <span>Log in with Telkom SSO</span>
                </button>
